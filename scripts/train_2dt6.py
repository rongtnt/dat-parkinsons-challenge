"""Lane 1c V2b: NetT6 on both channels (6ch) + mixup. 5 folds x 2 seeds, int8 state_dicts."""
...
from cnn2d import make_input6, NetT6  # noqa: E402
...
def build_inputs(folds):
    ...
    xz = {z: np.empty((len(rows), NCH, 64, 64), np.float32) for z in ZSHIFTS}
    for i, r in enumerate(rows):
        crop = np.asarray(crops[r], np.float32)
        for z in ZSHIFTS:
            xz[z][i] = make_input6(crop, z)
    return xz
...
def train_one(xz, y, tr, va, seed, fold, dev):
    ...
    model = NetT6(pretrained=True).to(dev)
    ...
    for ep in range(EPOCHS):
        ...
        for s in range(nstep):
            ...
            xb = np.empty((len(idx), NCH, 64, 64), np.float32)
            for z in ZSHIFTS:
                m = zb == z
                if m.any():
                    xb[m] = xz[z][idx[m]]
            ...
            if rng.random() < MIXUP_P:
                lam = float(rng.beta(MIXUP_A, MIXUP_A))
                perm = torch.randperm(xb.shape[0], device=dev)
...
def main():
    ...
    for k in sorted(folds.fold.unique()):
        ...
        out[["uid", "y", "fold", "family", "logit", "prob"]].to_csv(ROOT / "results/oof_2dt6.csv", index=False)
    ...
    json.dump({"a": a, "b": b}, open(SRC / "calib_2dt6.json", "w"))
    ...
    summ = {"n": int(m.sum()), "folds_done": done, "device": str(dev), "epochs": EPOCHS, "seeds": list(SEEDS),
            ...
            "arch": "NetT6(6ch, resnet18 truncated@layer3)", "label_smoothing_eps": 0.10,
            "mixup_p": MIXUP_P, "mixup_alpha": MIXUP_A}
    json.dump(summ, open(ROOT / "results/2dt6_summary.json", "w"), indent=1)
