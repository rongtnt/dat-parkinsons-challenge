"""Lane 1e: NetT6 (6ch, no mixup) with the top-5% (p95) noisiest blend-OOF scans dropped from TRAIN only."""
...
def main():
    ...
    for k in sorted(folds.fold.unique()):
        ...
        out[["uid", "y", "fold", "family", "logit", "prob"]].to_csv(ROOT / "results/oof_2dt6c5.csv", index=False)
    ...
    json.dump({"a": a, "b": b}, open(SRC / "calib_2dt6c5.json", "w"))
    ...
    summ = {"n": int(m.sum()), "folds_done": done, "device": str(dev), "epochs": EPOCHS, "seeds": list(SEEDS),
            ...
            "arch": "NetT6(6ch, no mixup, resnet18 truncated@layer3)", "label_smoothing_eps": 0.10,
            "noisy_loss_thresh": thresh, "noisy_pct": NOISY_PCT, "n_noisy_dropped": len(noisy)}
    json.dump(summ, open(ROOT / "results/2dt6c5_summary.json", "w"), indent=1)
