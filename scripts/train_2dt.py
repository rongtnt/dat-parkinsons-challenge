"""Lane 1b: 2.5D truncated ResNet18 (NetT), 5 folds x 2 seeds. Writes OOF, Platt calibration, int8 state_dicts."""
...
def main():
    ...
    for k in sorted(folds.fold.unique()):
        ...
        out[["uid", "y", "fold", "family", "logit", "prob"]].to_csv(ROOT / "results/oof_2dt.csv", index=False)
    ...
    json.dump({"a": a, "b": b}, open(SRC / "calib_2dt.json", "w"))
    ...
    json.dump(summ, open(ROOT / "results/2dt_summary.json", "w"), indent=1)
