"""Step C (revised): coronal 2.5D truncated-resnet18 (NetCorT), 5 folds x seeds {0,1}, int8 weights.
Recipe = LANE1_2D.md Step 3 with the coronal view, y-jitter, label smoothing eps=0.10 and int8 packing."""
...
OUT = ROOT / "submission_src/models/2dcor"
YSHIFTS = (-2, -1, 0, 1, 2)
SEEDS = (0, 1)
...
def main():
    ...
    summary = dict(device=device, arch="resnet18-trunc-layer3+Linear(256,1)", folds_done=done, seeds=list(SEEDS),
    ...
    if len(done) == 5:
        ...
        out.to_csv(ROOT / "results/oof_2dcor.csv", index=False)
        ...
        for tag in ("raw", "platt_scaled"):
            s = summary[tag]
            print(f"\n[cnn_cor {tag}] OOF logloss {s['logloss']:.5f}  AUC {s['auc']:.4f}")
    ...
    (ROOT / "results/2dcor_summary.json").write_text(json.dumps(summary, indent=1))
