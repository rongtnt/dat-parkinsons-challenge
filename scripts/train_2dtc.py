"""Lane 1c V2a: NetT trained with the top-3% noisiest blend-OOF scans dropped from TRAIN only."""
...
def main():
    ...
    noisy = set(blend.loc[blend.loss > NOISY_LOSS, "uid"])
    clean = ~folds.uid.isin(noisy).to_numpy()
    print(f"noisy uids dropped from train: {len(noisy)} (loss > {NOISY_LOSS})", flush=True)
    ...
    for k in sorted(folds.fold.unique()):
        ...
        out[["uid", "y", "fold", "family", "logit", "prob"]].to_csv(ROOT / "results/oof_2dtc.csv", index=False)
    ...
    json.dump({"a": a, "b": b}, open(SRC / "calib_2dtc.json", "w"))
    ...
    summ = {"n": int(m.sum()), "folds_done": done, "device": str(dev), "epochs": EPOCHS, "seeds": list(SEEDS),
            ...
            "noisy_loss_thresh": NOISY_LOSS, "n_noisy_dropped": len(noisy)}
    json.dump(summ, open(ROOT / "results/2dtc_summary.json", "w"), indent=1)
