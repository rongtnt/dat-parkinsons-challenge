"""Lane 1: 2.5D ResNet18, 5 folds x 2 seeds. Writes OOF, Platt calibration, fp16 state_dicts."""
...
from cnn2d import make_input, build_pretrained  # noqa: E402
...
def train_one(xz, y, tr, va, seed, fold, dev):
    ...
    t_smooth = torch.from_numpy((y * 0.95 + 0.025).astype(np.float32))
    ...
    torch.save({k: v.half() for k, v in model.state_dict().items()}, MODELS / f"f{fold}_s{seed}.pt")
    return model
...
def main():
    ...
    for k in sorted(folds.fold.unique()):
        ...
        lgs = []
        for seed in SEEDS:
            m = train_one(xz, y, tr, va, seed, int(k), dev)
        ...
        out[["uid", "y", "fold", "family", "logit", "prob"]].to_csv(ROOT / "results/oof_2d.csv", index=False)
    ...
    json.dump({"a": a, "b": b}, open(SRC / "calib_2d.json", "w"))
    ...
    summ = {"n": int(m.sum()), "folds_done": done, "device": str(dev), "epochs": EPOCHS, "seeds": list(SEEDS),
            ...
            "platt": {"a": a, "b": b}, "per_fold": per_fold, "per_family": per_fam, "seconds_per_fold": secs}
    json.dump(summ, open(ROOT / "results/2d_summary.json", "w"), indent=1)
